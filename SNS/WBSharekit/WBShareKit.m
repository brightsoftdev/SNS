...
- (void)startSinaOauthWithSelector:(SEL)_sSel 
          ...
          withNavigationController:(UINavigationController *)aNavigationController 
...
- (void)sinaLogout {
    NSUserDefaults *info = [NSUserDefaults standardUserDefaults];
    [info setValue:nil forKey:@"WBShareKit_sinaToken"];
    [info synchronize];
}

- (BOOL)isSinaLogin {
    NSUserDefaults *info = [NSUserDefaults standardUserDefaults];
    NSString *token = [info valueForKey:@"WBShareKit_sinaToken"];
...
- (void)sinaRequestTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"sina 获取未授权token失败 错误:%@",error);
}
...
- (void)sinaRequestTokenTicket:(OAServiceTicket *)ticket finishedWithData:(NSMutableData *)data {
    ...
    SNSWebViewController *webViewController = [[SNSWebViewController alloc] initWithUrl:url];
...
- (void)sinaAccessTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"sina 获取access token失败 错误:%@",error);

    NSUserDefaults *info = [NSUserDefaults standardUserDefaults];
    [self.delegate performSelector:NSSelectorFromString([info valueForKey:@"WBShareKit_ESel"]) withObject:error];
}
...
- (void)sinaAccessTokenTicket:(OAServiceTicket *)ticket finishedWithData:(NSMutableData *)data {
    ...
    DLog(@"获取access token:%@",responseBody);
    
    [self.navigationController popViewControllerAnimated:YES];
    
    [self.delegate performSelector:NSSelectorFromString([info valueForKey:@"WBShareKit_SSel"]) withObject:data];
...
- (void)txLogout {
    NSUserDefaults *info = [NSUserDefaults standardUserDefaults];
    [info setValue:nil forKey:@"WBShareKit_txToken"];
    [info synchronize];
...
- (BOOL)isTxLogin {
    NSUserDefaults *info = [NSUserDefaults standardUserDefaults];
    NSString *token = [info valueForKey:@"WBShareKit_txToken"];
    if (token && ![token isEqualToString:@""]) {
        return YES;
    }
    else 
...
- (void)txAccessTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"tx 获取access token失败 错误:%@",error);
...
- (void)doubanRequestTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"douban 获取未授权token失败 错误:%@",error);
}
...
- (void)doubanAccessTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"douban 获取access token失败 错误:%@",error);
...
- (void)twitterRequestTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"twitter 获取未授权token失败 错误:%@",error);
}
...
- (void)twitterAccessTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"twitter 获取access token失败 错误:%@",error);
...
- (void)wyAccessTokenTicket:(OAServiceTicket *)ticket failedWithError:(NSError *)error {
	DLog(@"wy 获取access token失败 错误:%@",error);
